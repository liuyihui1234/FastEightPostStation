...
public class GlideUtils {
    ...
    public static void showImageViewBlur(Context context, int errorimg, String url, final LinearLayout bgLayout) {
        Glide.with(context).load(url).asBitmap().error(errorimg)
                // 设置错误图片
                .diskCacheStrategy(DiskCacheStrategy.RESULT)
                // 缓存修改过的图片
 ...
 * Crop
 ...
 Color  ColorFilterTransformation, GrayscaleTransformation Blur
 BlurTransformation Mask  MaskTransformation
 */
    /**
     * （9）
     ...
     * @param imgeview 组件
     */
    public static void showImageViewToCircleAndBlur(Context context, int errorimg,
                                                    String url, ImageView imgeview) {
        Glide.with(context).load(url)
                .error(errorimg)// 设置错误图片
