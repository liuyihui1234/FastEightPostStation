...
public class BaseSPUtils {
    ...
    private static SharedPreferences getSharedPreferences(String shareName) {
        if (TextUtils.isEmpty(shareName)) {
            return PreferenceManager.getDefaultSharedPreferences(PostStationApplication.getInstance());
        } else {
            return PostStationApplication.getInstance().getSharedPreferences(shareName, Context.MODE_PRIVATE);
        }
    }
}
